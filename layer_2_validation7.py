...
Usage:  
        Execute/Run app7.py which launches a Web GUI
...
Author:
        Deon R. Murray — Purdue University
...
Notes:
        This is an improvement to the last Cable Validation Script that made an 2 SSH connections to the router, 
         per final result (a total of 24 router connections per POD cable calidation performed). 
        ...
        Recycled the even (BXX00) and odd (BXX01) router Shell keeping it open for the firewalls if any are inputed.
...
def open_conn1(device, user, pass1):
    ...
    try:
        cli.connect(f"{device}.XXX", username=user, password=pass1)
...
# open ssh connections for routers # XYZ informed me that routers are usually in production
# before Cable Validation, so I included a separate username and open_coon2 for rtrs.
...
def routerCheck(dev, username, password, shell1=""):
    ...
    if not shell1:
        ...
        out1 = command(shell, "show interface status | include notconnect", output=True) 
        return out1, shell, cli
    else:
        out1 = command(shell1, "show interface status | include notconnect", output=True) 
...
# after I was informed about the cable validation process typically being used with DA'ed routers, 
...
def even_rtr_check_lbs(user1, user2, key1, key2, devices, cable1):
    if devices.get_even_lb_list():
        ...
        for dev in devices.get_even_lb_list():
            ...
            cable1.store_rtr_port("".join(findall(r"Eth1/(\d{2}).*notconnect.*", diffs))) ### stores router port
