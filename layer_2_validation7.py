...
Usage:  
        ...
        Enter devices associated with a single pod; no need to enter a router
        Enter the username and passwords as indicated
        Run Validation
        Observe the backend

Author:
        XX — Purdue University | Summer 2025 Intern
...
Notes:
        ...
        Recycled the even (BXXXXX00) and odd (BXXXXX01) router Shell keeping it open for the firewalls if any FWs are inputed.
            - Now only 1 connection is opened to each router for the entire cable validation script.
            ** However, I have not confidently tested this version given that there are currently no more pre-DA LBs for testing as there are firewalls.
        ...
        Assumptions:
            >>>>> script assumes devices follow this format: XXXXXX00-01, XXXXXX00-05, and XXXXXX00-03 <<<<<<
...
# open ssh connections, returns cli channel and invoked shell; used for FWs and LBs
...
# alternative: an if statement that matches device for the router name: findall(r"B\d{2}.*(?:01|00)", var) then use username2, key2
def open_conn2(device, user, pass1):
    cli = SSHClient()
    cli.set_missing_host_key_policy(AutoAddPolicy())

    try:
        cli.connect(f"{device}.mgmt", username=user, password=pass1)
        shell = cli.invoke_shell()
        sleep(1)
        shell.recv(1000)
        sleep(1)
...
def get_rtrs_from_lb(key, user1, cableObj, devObj):
    ...
    if devObj.get_even_lb_list():
        ...
        even_check = True  # used to skip firewalls attempting to find router names if LB already stored it
        cli.close()
    else:
    ...
    if devObj.get_odd_lb_list():
        ...
        for i in out.splitlines():
            if findall(r"B\d{2}.*(01)",i): # finds the line of output containing the LB router connection
                ...
                for k in i.split():
                    # find and store the router
                    if findall(r"B\d{2}.*(01)", k):
                        devObj.set_odd_rtr(search(r"B\d{2}.*(01)", k).group()) # stores the odd router
        cli.close()
        odd_check = True  # used to skip firewalls attempting to find router names if LB already stored it
    else:
...
# This effectively reduces the authentication required from MFA by about half since one router shell is opened per router now
